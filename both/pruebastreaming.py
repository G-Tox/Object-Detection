...
labels_to_names = {0:'Casco',1:'Persona'}
...
while True:
    ...
    for j in range(len(df_ro)):
        ...
        cv2.putText(frame, caption, (b[0], b[1] - 10), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2) 
    ...
    for k in range(len(df_ro)):
        ...
        for l in range(len(df_ca)):
            ...
            if (coorde_ro[2]-coorde_ro[0])<280:
                if ((coorde_ro[1]-10)<coorde[1]) and ((((coorde_ro[3]-coorde_ro[1])/3)+coorde_ro[1])>coorde[3]) and (coorde_ro[0]<coorde[0]) and (coorde_ro[2]>coorde[2]):
                ...
                if ((coorde_ro[1]-10)<coorde[1]) and ((((coorde_ro[3]-coorde_ro[1])/2)+coorde_ro[1])>coorde[3]) and (coorde_ro[0]<coorde[0]) and (coorde_ro[2]>coorde[2]):
                    texto='Tiene el casco'
                    break
