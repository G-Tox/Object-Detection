...
model_path = os.path.join('snapshots', 'resnet50_csv_05_inference.h5')
...
labels_to_names = {0:'Persona',1:'Casco',2:'Cabeza'}
...
while True:
    ...
    for m in range(len(df_pe)):
    	coorde_pe1=df_pe.iloc[m][2]
        dt_fr_pe1=df_pe.iloc[m]
        texto=''

        for k in range(len(df_ro)):
        	coorde_ro1=df_ro.iloc[k][2]

       		if ((coorde_pe1[0]-5)<coorde_ro1[0]) and ((coorde_pe1[1]-10) <coorde_ro1[1]) and ((coorde_pe1[2]+10) > coorde_ro1[2]) and (coorde_pe1[3]+10>coorde[3]):
